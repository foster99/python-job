...
class Restaurants_db_manager:
    ...
    def drop_all_tables(self):
        ...
        drop1 = self.mySqlDB.drop_table("Restaurant_Segment_Association")
        drop2 = self.mySqlDB.drop_table("Restaurant")
        drop3 = self.mySqlDB.drop_table("Segment")

        return drop1 and drop2 and drop3

    def insert_into_restaurants(self, columnNames, restaurants):
        
        return self.mySqlDB.insertInTable('Restaurant', columnNames, restaurants, format_values=True)
    
    def insert_into_segmetns(self, columnNames, segments):
        
        return self.mySqlDB.insertInTable('Segment', columnNames, segments, format_values=True)
        
    
    def insert_into_rs_association(self, columnNames, rs_associations):
        
        return self.mySqlDB.insertInTable('Restaurant_Segment_Association', columnNames, rs_associations, format_values=True)
    ...
    def import_restaurants_to_mySqlDB(self, path):
        ...
        with open(path) as json_restaurant_file:
            ...
            for restaurant in data:
                ...
                uidentifier = "\'" + str(restaurant['uidentifier']).replace('\'', '\\\'').replace(
                    '\\\\', '\\') + "\'" if restaurant['uidentifier'] != None else "NULL"

                if uidentifier in restaurantUIDs:
                    continue
                restaurantUIDs.add(uidentifier)

                name = "\'" + str(restaurant['name']).replace('\'', '\\\'').replace(
                    '\\\\', '\\') + "\'" if restaurant['name'] != None else "NULL"
                street_address = "\'" + str(restaurant['street_address']).replace('\'', '\\\'').replace(
                    '\\\\', '\\') + "\'" if restaurant['street_address'] != None else "NULL"
                latitude = float(
                    restaurant['latitude']) if restaurant['latitude'] != None else "NULL"
                longitude = float(
                    restaurant['longitude']) if restaurant['longitude'] != None else "NULL"
                city_name = "\'" + str(restaurant['city_name']).replace('\'', '\\\'').replace(
                    '\\\\', '\\') + "\'" if restaurant['city_name'] != None else "NULL"
                popularity_rate = float(
                    restaurant['popularity_rate']) if restaurant['popularity_rate'] != None else "NULL"
                satisfaction_rate = float(
                    restaurant['satisfaction_rate']) if restaurant['satisfaction_rate'] != None else "NULL"
                total_reviews = int(
                    restaurant['total_reviews']) if restaurant['total_reviews'] != None else "NULL"
                average_price = float(
                    restaurant['average_price']) if restaurant['average_price'] != None else "NULL"
    ...
    def import_segments_to_mySqlDB(self, path):
        ...
        with open(path) as json_segments_file:
            ...
            for segment_dict in data:
                ...
                uidentifier = "\'" + str(segment_dict['uidentifier']).replace('\'', '\\\'').replace(
                    '\\\\', '\\') + "\'" if segment_dict['uidentifier'] != None else "NULL"

                if uidentifier in segmentUIDs:
                    continue
                segmentUIDs.add(uidentifier)

                name = "\'" + str(segment_dict['name']).replace('\'', '\\\'').replace(
                    '\\\\', '\\') + "\'" if segment_dict['name'] != None else "NULL"
            ...
            for segment_dict in data:
                ...
                # Use segment uniquenes constraint to avoid PRIMARY KEY violation
                segmentUID = "\'" + str(segment_dict['uidentifier']).replace('\'', '\\\'').replace(
                    '\\\\', '\\') + "\'" if segment_dict['uidentifier'] != None else "NULL"

                if segmentUID in segmentUIDs:
                    continue
                ...
                for restaurant in segment_dict['restaurants']:
                    ...
                    restaurantUID = "\'" + str(restaurant).replace('\'', '\\\'').replace(
                        '\\\\', '\\') + "\'" if restaurant != None else "NULL"

                    # Use restaurant uniquenes constraint to avoid PRIMARY KEY violation
                    if restaurantUID in restaurantsUIDs:
                        continue
                    restaurantsUIDs.add(restaurantUID)

                    values.append([segmentUID, restaurantUID])

            self.mySqlDB.insertInTable(
                "Restaurant_Segment_Association", column_names, values)
            ...
            self.mySqlDB.set_derived_value(
                'Segment', 'average_popularity_rate', avgPopularityQuery, table_alias='target')
            self.mySqlDB.set_derived_value(
                'Segment', 'average_satisfaction_rate', avgSatisfactionQuery, table_alias='target')
            self.mySqlDB.set_derived_value(
                'Segment', 'average_price', avgPriceQuery, table_alias='target')
            self.mySqlDB.set_derived_value(
                'Segment', 'total_reviews', totalReviewsQuery, table_alias='target')
    ...
    def query_restaurants(self, params):
        ...
        for attribute in params:
            for operator in params[attribute]:
                ...
                symbol_operator = self._get_operator_symbol(
                    operator, null_is_involved=(value is None))
    ...
    def get_all_restaurants(self, dictionary=False):

        return self.mySqlDB.consultant_query("SELECT * FROM Restaurant R;", dict_=dictionary)

    def get_all_restaurants(self, dictionary=False):

        return self.mySqlDB.consultant_query("SELECT * FROM Restaurant R;", dict_=dictionary)

    def get_all_restaurants_joined_to_segments(self, dictionary=False):

        query = (
            "SELECT *\n"
            "FROM Restaurant R LEFT OUTER JOIN Restaurant_Segment_Association RS ON R.uidentifier = RS.restaurantUID;"
        )

        return self.mySqlDB.consultant_query(query, dict_=dictionary)

    def get_segments_from_restaurant(self, restaurantUID, dictionary=False):

        query = (
            "SELECT RS.segmentUID\n"
            "FROM Restaurant_Segment_Association RS\n"
            f"WHERE RS.restaurantUID = '{restaurantUID}';"
        )
    ...
    def export_all_data(self) -> str:
        ...
        for segment in list_of_segments:
            ...
            segments[segmentUID] = {
                "name": name, "size": size, "uidentifier": segmentUID, "restaurants": []}
        ...
        for association in list_of_associations:
            ...
            segments[segmentUID]["restaurants"].append(
